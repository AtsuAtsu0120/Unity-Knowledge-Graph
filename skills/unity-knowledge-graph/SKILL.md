...
グラフは古いことがある。**型/依存/影響など構造に関する質問に答える前に `status` で鮮度を確認**し、
古ければ自分で `index` してから答える。`index` は変更が無ければ即スキップ（`upToDate:true`）なので軽い。

```bash
ukg status <UnityProjectPath>          # fresh:false なら古い、reviewPending>0 なら要再確認
# fresh:false の場合:
ukg index <UnityProjectPath>           # 増分更新（変更ファイルのみ・意味エッジは保持）
```

判断の指針:
- `indexed:false` → 一度も構築されていない。`ukg index` する。
- `fresh:false` → コードが変わっている。`ukg index` してから構造の質問に答える。
- `fresh:true` → そのまま答えてよい。
- `reviewPending>0` → 構造変更で意味エッジが古くなった可能性。下記リフレクションへ。
...
構造（コード）が変わると、あなたが付けた意味エッジが古くなる。`index` は変更ファイルに係る意味
エッジを自動で `needsReview` 化する（stale伝播）。**時々 `reflect` を回して意味層を手入れする**のが
あなたの仕事。

```bash
ukg reflect            # 要再確認/低confidence/孤児/重複概念/未整理コミュニティをまとめて取得
ukg sem review         # 構造変更で要再確認になった意味エッジだけ
```

`reflect` の各カテゴリへの対応:
- **needsReview / sem review** — コードを読み直し、まだ妥当なら
  `ukg sem confirm --from … --to … --rel …`、変わったなら `ukg sem add … --supersede` で更新。
- **lowConfidence** — 低確信のエッジ。再検証して confidence を上げる/下げる、または supersede。
- **staleNodes** — 消えた型に係る知識。係っていた意味エッジを別ノードへ張り直すか無効化。
- **duplicateConcepts** — 同名概念。1つに統合（不要な方を作り直さない）。
- **uncuratedCommunities** — 機械生成のコミュニティ。`concept add` で意味のある概念に命名・集約。

原則: **構造（継承/参照/呼び出し/依存）は自動で最新になる。あなたが手入れするのは意味層だけ。**
構造の事実を `sem add` で二重に持たない。
