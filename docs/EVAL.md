...
dotnet test --filter EvalHarness

# スコアカードはファイルに出る（フラグ不要）
cat tests/Ukg.Eval/eval-report.txt

# ターミナルにインライン表示したい場合（xUnit の出力は detailed ロガーが必要）
dotnet test --filter EvalHarness --logger "console;verbosity=detailed"
```
